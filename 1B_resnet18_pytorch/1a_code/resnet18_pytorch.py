"""
ResNet-18 Training on Full PlantVillage Dataset using PyTorch
...
class ResNet18(nn.Module):
    ...
    def _make_layer(self, in_channels, out_channels, num_blocks, stride):
        ...
        # Create downsample if needed (channels or stride changes)
...
class PlantVillageResNet18:
    """
    Train ResNet-18 on full PlantVillage dataset using PyTorch
    """
    
    def __init__(self, data_path, output_path='resnet18_results'):
        """
        ...
        logger.info(f"Initialized ResNet-18 trainer with output path: {output_path}")
        logger.info(f"Using device: {device}")
    ...
    def train_model(self):
        """Train ResNet-18 on full dataset"""
        logger.info("="*60)
        logger.info("Training ResNet-18 on full PlantVillage dataset (PyTorch)")
        logger.info("="*60)
        
        # Check if model already exists
        model_path = self.models_path / 'resnet18_full.pth'
        results_path = self.results_path / 'resnet18_full_results.json'
        history_path = self.training_logs_path / 'resnet18_full_history.pkl'
        
        if model_path.exists() and results_path.exists() and history_path.exists():
            logger.info("✓ ResNet-18 model already exists. Loading results...")
        ...
        # Build model
        logger.info("Building ResNet-18 architecture...")
        ...
        # REMOVED verbose parameter - FIXED
        ...
        # Save final model
        torch.save(model.state_dict(), self.models_path / 'resnet18_full_final.pth')
        ...
        logger.info("✓ ResNet-18 training complete!")
    ...
    def plot_confusion_matrix(self, conf_matrix):
        ...
        plt.title('ResNet-18 - Confusion Matrix (Full Dataset)')
    ...
    def plot_training_history(self, history):
        ...
        axes[0].set_title('ResNet-18 - Accuracy', fontsize=14, fontweight='bold')
        ...
        axes[1].set_title('ResNet-18 - Loss', fontsize=14, fontweight='bold')
    ...
    def visualize_features(self, model):
        ...
        plt.title('t-SNE Visualization - ResNet-18 Features', fontsize=14, fontweight='bold')
    ...
    def plot_results_summary(self, results, history):
        ...
        ax1.set_title('ResNet-18 Performance Metrics')
        ...
        plt.suptitle('ResNet-18 Training Results - PlantVillage Full Dataset', 
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(self.figures_path / 'resnet18_results_summary.png', dpi=300, bbox_inches='tight')
        plt.close()
        logger.info(f"Results summary saved to {self.figures_path / 'resnet18_results_summary.png'}")
    ...
    def generate_report(self, results, history):
        ...
        report = []
        report.append("# ResNet-18 Training Report - PlantVillage Dataset (PyTorch)")
        ...
        report.append(f"\n## Model Architecture")
        report.append(f"- **Architecture:** ResNet-18 (He et al., 2016)")
        ...
        report.append(f"- **Early Stopping Patience:** 15")
        report.append(f"- **Optimizer:** Adam")
        report.append(f"- **Loss Function:** Cross Entropy Loss")
        report.append(f"- **Learning Rate Scheduler:** ReduceLROnPlateau (patience=7, factor=0.5)")
        ...
        if results['accuracy'] > 0.9:
            report.append(f"- **Excellent Performance:** ResNet-18 achieved {results['accuracy']*100:.2f}% accuracy on the test set")
        elif results['accuracy'] > 0.8:
            report.append(f"- **Good Performance:** ResNet-18 achieved {results['accuracy']*100:.2f}% accuracy")
        
        report.append(f"- **Training Stability:** The model showed consistent learning with validation accuracy improving over epochs")
        report.append(f"- **Efficiency:** ResNet-18 with {results['parameters']['total']:.1f}M parameters provides good accuracy-to-parameter ratio")
        report.append(f"- **PyTorch Implementation:** Successfully implemented ResNet-18 from scratch using PyTorch framework")
        
        # Recommendations
        report.append(f"\n## Recommendations")
        report.append(f"\nBased on the analysis:")
        report.append(f"1. **ResNet-18 is effective** for plant disease classification on this dataset")
        report.append(f"2. **Consider deeper ResNet variants** (e.g., ResNet-50) if higher accuracy is required")
        report.append(f"3. **Transfer learning** could potentially improve performance further")
        report.append(f"4. **Model size ({results['parameters']['total']:.1f}M parameters)** is efficient for deployment")
...
if __name__ == "__main__":
    ...
    # Generate report
    trainer.generate_report(results, history)
